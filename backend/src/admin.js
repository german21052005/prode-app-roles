
import { Router } from 'express';
import { pool } from '../db.js';
import { requireAuth } from './middleware/authMiddleware.js';
import { requireRole } from './middleware/roleMiddleware.js';

const router = Router();

function sign(h,a){ if(h>a) return 'L'; if(h<a) return 'V'; return 'E'; }

// Recalcular puntajes manualmente (global o filtrado)
// POST /admin/recalc { match_id?: number, round?: number }
router.post('/recalc', requireAuth, requireRole('admin'), async (req, res) => {
  const { match_id, round } = req.body || {};
  let filter = '';
  const params = [];
  if (match_id) { filter = 'WHERE m.id = $1'; params.push(match_id); }
  else if (round) { filter = 'WHERE m.round = $1'; params.push(round); }

  const q = `
    SELECT p.user_id, p.match_id, p.home_goals AS ph, p.away_goals AS pa,
           m.final_home_goals AS fh, m.final_away_goals AS fa
    FROM predictions p
    JOIN matches m ON m.id = p.match_id
    ${filter}
    AND m.final_home_goals IS NOT NULL AND m.final_away_goals IS NOT NULL
  `;

  const { rows } = await pool.query(q, params);
  let updated = 0;
  for (const r of rows) {
    let pts = 0; let reason = [];
    if (sign(r.ph, r.pa) === sign(r.fh, r.fa)) { pts += 3; reason.push('signo'); }
    if (r.ph === r.fh && r.pa === r.fa) { pts += 3; reason.push('exacto'); }
    await pool.query(
      `INSERT INTO scores(user_id, match_id, points, reason)
       VALUES($1,$2,$3,$4)
       ON CONFLICT (user_id, match_id)
       DO UPDATE SET points=$3, reason=$4`,
      [r.user_id, r.match_id, pts, reason.join('+') || null]
    );
    updated++;
  }

  res.json({ ok: true, updated });
});

// Ver auditoría (últimos 200 eventos o por match)
router.get('/audit', requireAuth, requireRole('admin'), async (req, res) => {
  const { match_id } = req.query;
  let q = `SELECT a.*, u.username AS by_username
           FROM match_audit a
           LEFT JOIN users u ON u.id = a.by_user`;
  const p = [];
  if (match_id) { q += ' WHERE a.match_id = $1'; p.push(match_id); }
  q += ' ORDER BY a.at DESC LIMIT 200';
  const r = await pool.query(q, p);
  res.json(r.rows);
});

export default router;
